...
- Lecture des projets Optix : toujours par `common/optix` (`project` pour le dossier et le `.optix`, `text` pour les fichiers à l'octet près, `scanner` pour l'index des nœuds d'un fichier, `tree` pour l'arbre de tout le projet sans PyYAML). Toute évolution de `tree` garde l'égalité nœud par nœud avec PyYAML (`tests/test_optix_tree.py`) ; un fichier hors du format généré par FT Optix est relu par PyYAML.
...
- Un nouvel outil = un paquet `modules/<id>/` + une entrée `ModuleSpec` dans `modules/__init__.py`. La coquille (`shell/`) ne connaît aucun outil par son nom : ce qu'elle propose pour un outil passe par les champs de `ModuleSpec` (`opens_projects`, `controller_action`…).
