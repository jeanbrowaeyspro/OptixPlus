...
- Thème : couleurs uniquement depuis `common.theme` ; un widget suit les changements de thème par `theme.follow(self, self._slot)` (jamais `manager().changed.connect`, qui survit au widget détruit) ; ne jamais fixer un fond sans fixer le texte (Windows de Jean en mode sombre). Aucune police ni feuille de style globale posée par un outil.
