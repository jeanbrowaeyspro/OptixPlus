...
- Langue par défaut : celle de Windows (français pour toute variante fr-*, anglais sinon). Le changement de langue est appliqué à chaud en **reconstruisant la fenêtre** : un outil doit supporter d'être fermé et rouvert sans perte (état enregistré, `can_close()` s'il a un travail en cours). Ne jamais connecter une fonction anonyme à un signal durable (service) pour mettre à jour un widget ou une action : utiliser une méthode d'un QObject (déconnexion automatique à sa destruction), par exemple `CheckedSync`.
