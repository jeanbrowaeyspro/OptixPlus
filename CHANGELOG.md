...
### Ajouté
- Paramètres › Général, rubrique « Messages » : deux cases, cochées par défaut, pour afficher ou non la bulle qui signale qu'OptixPlus reste actif après la fermeture de sa fenêtre, et le message qui explique pourquoi Impr. écran ne fait rien quand OptixPlus est lancé en administrateur.

### Modifié
- Notifications Windows : l'en-tête affiche « OptixPlus » (avec son icône). Le nom de l'auteur n'apparaît plus que dans la fenêtre À propos : l'éditeur (Applications installées) et les propriétés de l'exécutable indiquent « OptixPlus ».
- Titre de la fenêtre et de la barre des tâches : « OptixPlus - Outil » (le nom de l'application en tête, séparé par un simple tiret).
- Lecteur de logs, Contrôle des liens et Comparaison : plus de trait sur les séparations entre les zones (elles restent déplaçables) ; un peu plus d'espace entre le tableau et le détail dans le Lecteur de logs et le Contrôle des liens.
