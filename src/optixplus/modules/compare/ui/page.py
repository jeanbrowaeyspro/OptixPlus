...
class ComparePage(QWidget):
    ...
    def _on_success(self, comparison: Comparison) -> None:
        ...
        if self._plan_to_replay is not None:
            ...
            message += " " + tr("Decisions kept: {n} difference(s)").format(n=plan.nb_pris())
    ...
    def replay_last_plan(self) -> list[str] | None:
        ...
        self._adopt_plan(plan)
        self.show_message(
            tr("Decisions replayed: {taken} difference(s), {lost} not found again.").format(
                taken=plan.nb_pris(), lost=len(perdus)
            )
        )
        if perdus:
            QMessageBox.warning(
                self, tr("Decisions partly replayed"), tr("Differences not found:") + "\n" + "\n".join(perdus)
            )
        return perdus
